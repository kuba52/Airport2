...
public sealed class SearcherService
    {
        ...
        public IReadOnlyList<Flight> GetFlights(int skad, int dokad, DateTime kiedy)
        {
            var list = _dbConnection.Query<Flight, Plane, Flight>(
                @"SELECT f.id AS arrivalId, f.start_time AS startTime, f.arrival_time AS arrivalTime, c.destination AS dest, c2.destination AS From, p.id as planeId, p.company AS comp, p.capacity
            ...
            var list2 = _dbConnection.Query<FlightWithChange, Plane, FlightWithChange>(
                ...
                WHERE start = @star AND flights.start_time > @k)
                    SELECT z.id AS arrivalId, z.start_time AS startTime, f.arrival_time AS arrivalTime, c.destination AS dest, c3.destination AS from, z.arrival_time AS stStopover, c2.destination AS stop, f.start_time AS spStopover, f.id AS idsecond, p.id AS planeId, p.company AS comp, p.capacity
        ...
        public void Purchase(int id, int account){
            _dbConnection.Query("INSERT INTO passengers(flight_id, account_id) VALUES(@i, @a)", new{i = id, a = account});
        }

        public void Purchase(int id1, int id2, int account){
            _dbConnection.Query("INSERT INTO passengers(flight_id, account_id) VALUES (@i, @a), (@id, @a)",
